function [varFit, R] = pixelfitK_GPU_Vectorized(Texp, K_matrix, functiontype)
% PIXELFITK_GPU_VECTORIZED Vectorized Trust-Region-Reflective optimizer on GPU.
...
%   Texp        : [nT x 1] vector of exposure times (gpuArray)
%   K_matrix    : [nT x N] matrix containing N pixels (gpuArray)
%   functiontype: 's' (spatial) or 't' (temporal)

    [nT, num_pixels] = size(K_matrix);

    % Slow dynamics data should have the first 7 timepoints (< 1000 ms) dropped
    % by the caller.  Assert here so misuse is caught early.
    assert(nT <= 28, 'pixelfitK_GPU_Vectorized: expected <=28 timepoints (7 dropped). Got %d.', nT);

    % 1. Initialization Parameters Per Pixel
    % beta fixed per-pixel to K(1)^2, matching pixelfitK_singleTauFit CPU fitter
    beta  = K_matrix(1, :).^2;
    ...
    [K_pred, J_rho, J_tauC] = evaluate_jacobian(T_3D, beta, rho, tauC, functiontype);
    ...
    for iter = 1:max_iters
        ...
        tauC_t = reflect_bounds(tauC_t, 1e-2,  40000.0);  % matches CPU slow fitter upper bound

        % Evaluate model at trial point
        [K_t, J_rho_t, J_tauC_t] = evaluate_jacobian(T_3D, beta, rho_t, tauC_t, functiontype);
        res_t    = K_3D - K_t;
...
%% --- ANALYTICAL JACOBIAN CORE HELPER ---

function [K, J_rho, J_tauC] = evaluate_jacobian(T, beta, rho, tauC, type)
    ...
    K_safe      = max(K, 1e-8);
    dK_dV       = 0.5 .* beta_3D ./ K_safe;

    % Analytical Jacobians via chain rule
    J_rho        = dK_dV .* dV_drho;
